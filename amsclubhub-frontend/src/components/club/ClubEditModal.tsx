...
export default function ClubEditModal({
	...
	return (
		<Dialog open={isOpen} onOpenChange={(open) => !open && handleCloseModal()}>
			<DialogContent className="sm:max-w-lg bg-card text-foreground border border-border max-h-[90vh] overflow-y-auto p-6 rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
				{/* MÀN HÌNH CẮT ẢNH */}
				{cropTarget && tempImageSrc ? (
					<div className="space-y-4">
						...
						{/* Khung chứa Cropper */}
						<div className="relative w-full h-[320px] bg-muted rounded-xl overflow-hidden border border-border">
							<Cropper
								image={tempImageSrc}
								crop={crop}
								zoom={zoom}
						...
						<div className="flex items-center gap-3 px-2">
							<span className="text-xs font-medium text-muted-foreground shrink-0">Thu phóng:</span>
							<input
								type="range"
								min={1}
								max={3}
						...
						<form onSubmit={onSubmit} className="space-y-4 mt-2">
							...
							<div className="relative pt-2 pb-8">
								...
								<div
									...
									className="relative w-full aspect-[16/5] bg-muted rounded-xl overflow-hidden border border-border cursor-pointer group"
								>
									<img
									...
									<div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
										<div className="p-3 rounded-full bg-background/80 text-foreground hover:bg-background transition-colors backdrop-blur-sm shadow-md">
								...
								<div className="absolute left-4 -bottom-2">
									<div
										...
										className="relative w-20 h-20 rounded-full border-4 border-card bg-muted shadow-lg overflow-hidden cursor-pointer group"
									>
										<img
											...
											className="w-full h-full object-cover brightness-90 group-hover:brightness-75 transition-all"
										/>
										<div className="absolute inset-0 flex items-center justify-center bg-background/60 opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-[1px]">
							...
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground">Tên câu lạc bộ</label>
								<Input
							...
							{/* Code */}
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground">Mã câu lạc bộ</label>
								<Input
									className="bg-background border-border focus:border-ring text-foreground placeholder:text-muted-foreground/60 rounded-lg focus-visible:ring-0 focus-visible:ring-offset-0"
							...
							{/* Category */}
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground">Lĩnh vực / Thể loại</label>
								<div className="relative">
									{/* Ô hiển thị chính */}
									<button
										type="button"
										onClick={() => setIsCategoryOpen((prev) => !prev)}
										className={`w-full h-8.5 bg-muted/50 border border-border text-left px-3 text-sm transition-all flex items-center justify-between cursor-pointer focus:outline-none ${isCategoryOpen
											? 'rounded-t-lg border-primary border-b-border bg-muted'
											: 'rounded-lg hover:border-border/80 focus:border-primary'
											} ${!formData.category ? 'text-muted-foreground/60' : 'text-foreground'}`}
									>
										<span>{formData.category || '-- Chọn thể loại --'}</span>
										<ChevronDown
											className={`w-4 h-4 text-muted-foreground transition-transform duration-200 ${isCategoryOpen ? 'rotate-180' : ''
												}`}
										/>
									</button>
									...
									{isCategoryOpen && (
										<>
											...
											<div className="absolute top-full left-0 right-0 -mt-[1px] z-20 bg-muted border border-t-0 border-border rounded-b-lg shadow-xl overflow-hidden">
												{['Thể thao', 'Nghệ thuật', 'Học thuật', 'Xã hội'].map((item) => (
													<button
														...
														className={`w-full text-left px-2 py-1.5 text-sm transition-colors hover:bg-accent hover:text-accent-foreground cursor-pointer last:rounded-b-lg ${formData.category === item ? 'bg-accent font-medium text-accent-foreground' : 'text-muted-foreground'
															}`}
													>
														{item}
													</button>
							...
							{/* Mô tả câu lạc bộ */}
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground">Mô tả câu lạc bộ</label>
								<Textarea
									rows={3}
									className="bg-background border-border focus:border-ring text-foreground placeholder:text-muted-foreground/60 rounded-lg focus-visible:ring-0 focus-visible:ring-offset-0 resize-none"
							...
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground">Link Fanpage Facebook</label>
								<Input
									className="bg-background border-border focus:border-ring text-foreground placeholder:text-muted-foreground/60 rounded-lg focus-visible:ring-0 focus-visible:ring-offset-0"
