...
import { Camera, Image as ImageIcon, Loader2, ArrowLeft, X, CheckCircle2 } from 'lucide-react';
  ...
  const handleApplyCrop = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (serverPath) {
        ...
        triggerToast?.('Tải ảnh thất bại: Backend không trả về đường dẫn file hợp lệ', 'error');
      }
    } catch (err: any) {
      console.error('Lỗi crop/upload ảnh:', err);
      triggerToast?.(err?.response?.data?.detail || err?.response?.data?.message || 'Có lỗi xảy ra khi tải ảnh', 'error');
    } finally {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleCloseModal()}>
      <DialogContent className="sm:max-w-lg bg-black text-white border border-zinc-800 max-h-[90vh] overflow-y-auto p-6 rounded-2xl shadow-2xl">
        ...
        {cropTarget && tempImageSrc ? (
          ...
          /* FORM CHỈNH SỬA THÔNG TIN CLB (TWITTER / X STYLE DARK MODE) */
          <>
            ...
            <form onSubmit={onSubmit} className="space-y-4 mt-2">
              ...
              {/* DỮ LIỆU NHẬP LIỆU (STYLED INPUTS) */}
              ...
              <div className="space-y-1">
                ...
                <Input
                  className="bg-black border-zinc-800 focus:border-zinc-400 text-white placeholder:text-zinc-600 rounded-lg focus-visible:ring-0 focus-visible:ring-offset-0"
                  placeholder="Ví dụ: Nghệ thuật, Thể thao..."
