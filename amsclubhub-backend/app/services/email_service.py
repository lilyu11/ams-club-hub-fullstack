...
def send_html_email_sync(to_email: str, subject: str, html_content: str, idempotency_key: str | None = None) -> bool:
	"""Gửi email qua Brevo hoặc SMTP — phiên bản DONG BO cho scheduler (tránh asyncio.run)."""
	if settings.BREVO_API_KEY:
		try:
			sender_name, sender_email = sender_parts()
			headers = {"api-key": settings.BREVO_API_KEY, "Content-Type": "application/json"}
			...
			with httpx.Client(timeout=settings.EMAIL_TIMEOUT_SECONDS) as client:
				...
				if response.status_code not in (200, 201):
					print(f"[Brevo-Sync] Error {response.status_code}: {response.text}")
					return False
			return True
		except Exception as exc:
			print(f"[Brevo-Sync] Failed to send email to {to_email}: {exc}")
			return False

	if not settings.SMTP_USER or not settings.SMTP_PASSWORD:
		print(f"[Warning-Sync] Email provider is not configured; skipped email to {to_email}")
		return False
	...
	try:
		...
		print(f"[SMTP-Sync] Xảy ra lỗi khi gửi email tới {to_email}: {exc}")
		return False
...
def send_reminder_email_sync(
	...
	<html>
		...
		<div style="max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 20px;">
			...
			<p>Bạn đã đăng ký nhận thông báo nhắc nhở bài viết <strong>{post_title}</strong>:</p>
			<div style="background-color: #f9f9f9; border-left: 4px solid #e53935; padding: 15px; margin: 15px 0;">
				<p style="margin: 0; color: #d32f2f;"><strong>{email_message or ''}</strong></p>
			</div>
			<p>Hãy nhanh tay hoàn thiện đơn đăng ký trước khi cổng đăng ký đóng lại nhé!</p>
			<p style="text-align: center; margin-top: 25px;">
				<a href="{action_url or '/'}" style="background-color: #1976d2; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; display: inline-block;">ĐĂNG KÝ NGAY</a>
	...
	"""
	return send_html_email_sync(to_email, subject, html_content, idempotency_key)
